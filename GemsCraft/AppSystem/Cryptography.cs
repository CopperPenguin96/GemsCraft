...
{
    /// <summary>
    /// Uses Craft.NET's (ddevault) Cryptography class.
    /// </summary>
    ...
    public class AsnKeyBuilder
    {
        ...
        {
            return !IsEmpty(octets) && octets.All(t => 0 == t);
        }
        ...
        public class AsnMessage
        {
            ...
            public int Length => mOctets?.Length ?? 0;
        ...
        public class AsnType
        {
            ...
            public byte[] Tag => mTag ?? Empty;

            public byte[] Length => mLength ?? Empty;
            ...
            private static byte[] Concatenate(IReadOnlyCollection<byte[]> values)
            {
                ...
                int length = values.Where(b => null != b).Sum(b => b.Length);
    ...
    class AsnParser
    {
        ...
        public AsnParser(IReadOnlyCollection<byte> values)
        {
            octets = new List<byte>(values.Count);
        ...
        internal int NextSequence()
        {
            ...
            try
            {
                ...
                int length = GetLength();
                if (length <= RemainingBytes()) return length;
                {
                    StringBuilder sb = new StringBuilder("Incorrect Sequence Size. ");
                    sb.AppendFormat("Specified: {0}, Remaining: {1}",
                        length.ToString(CultureInfo.InvariantCulture),
